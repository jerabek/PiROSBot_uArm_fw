...
unsigned char myMoveTo(double x, double y, double z, double speed)
{
...
unsigned char moveTo(double x, double y, double z, double speed)
{
	...
	if( ((x > -120) && (x < 120)) && (y < 150) && (z <85) ) // front
	{
		return OUT_OF_RANGE_NO_SOLUTION;
	}
	else if(z < 25) // left side
	{
		return OUT_OF_RANGE_NO_SOLUTION;
	}
...
unsigned char setServoAngle(unsigned char servoNumber, double angle)
{
	...
	if(servoNumber < 3) // servos that modify x,y,z
	{
		// check angle!!
		rot = getServoAngle(0);
		left = getServoAngle(1);
		right = getServoAngle(2);
		switch(servoNumber)
		{
			case 0: 
				rot = angle; 
		...
		debugPrint("Servo %f, %f, %f / Cord %f, %f, %f", rot, left, right, x, y, z);
	...
	while (mCurStep >= 0 && mCurStep < mTotalSteps)
	{
		...
		if((millis() - mStartTime) >= (mCurStep * mTimePerStep)) 
		{
			...
			if (controller.limitRange(mPathX[mCurStep], mPathY[mCurStep], mPathZ[mCurStep]) != OUT_OF_RANGE_NO_SOLUTION)
			{
				...
				controller.readServoAngles(x,y,z,true);
				debugPrint("0=%f, 1=%f, 2=%f", x, y, z);
				controller.getCurrentXYZ(x, y, z);
				debugPrint("X=%f, Y=%f, Z=%f", x, y, z);
